...
df<- df[,-1]
df[,c(89:340)]<- lapply(df[,c(89:340)], as.factor)
...
map.df <- trainSplit[,-333]
map.df[,87:336]<-lapply(trainSplit[,87:336], as.numeric.factor)
...
```{r}
train.map.df <- train.set[,-333]
train.map.df[,87:336]<-lapply(train.set[,87:336], as.numeric.factor)
...
test.map.df[,87:336]<-lapply(test.set[,87:336], as.numeric.factor)
