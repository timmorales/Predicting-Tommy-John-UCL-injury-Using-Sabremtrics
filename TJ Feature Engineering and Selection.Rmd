...
doesnt.throw<- function(x){
  x<-ifelse(is.na(x), "Does_Not_Throw", x)
}
...
near zero variance variables that we can and should drop. We will keep 13, 14,18,28 
```{r}
colnames(df)[nearZeroVar(df)]
nzv<-nearZeroVar(df)[-c(13,14,18,28,34,35,36)]
colnames(df)[nzv]
df<- df[, -nzv]
...
write.csv(df.juice, "/Users/timmorales/Desktop/STAT 6341/Tommy John Project/Data/ModelReadyData")
