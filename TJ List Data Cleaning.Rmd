...
#keep everyone up to september 2015 
...
#if they got surgery after august or during august in that year
# we can use the same season. 
#if before august, we use year prior.
mlb_only_tj_list$Year_for_stats<-ifelse(mlb_only_tj_list$Mon >= 8, mlb_only_tj_list$Year,mlb_only_tj_list$Year - 1) 
...
df<-df.total[ , colSums(is.na(df.total)) < 500]
df<-dplyr::select_if(df, is.numeric)
df<-na.omit(df)
df.scaled <- as.data.frame(scale(df[,-c(92)]))
df.scaled$TJ<-df$TJ

df.map<-umap(df.scaled[,-c(92)])
...
df.scaled$TJ<-as.factor(df.scaled$TJ)
levels(df.scaled$TJ)= c("No", "Yes")
umap.plot(df.map,df.scaled$TJ)
```
